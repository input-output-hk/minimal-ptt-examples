...
-- | Players
alice, bob, jane :: Cooked.Wallet
alice = Cooked.wallet 2
bob = Cooked.wallet 3
jane = Cooked.wallet 4
...
aliceBobandJanePlay ::
  ...
  BuiltinByteString ->
  m ()
aliceBobandJanePlay setup salt secret = do
  let hashedSecret = Lib.hashSecret secret (Just salt)
  (initLottoRef, initLotto) <- Lotto.open setup hashedSecret salt
  (authenticatedLottoRef, authenticatedLotto, seal) <-
    Lotto.mintSeal initLottoRef (initLotto ^. Cooked.outputValueL)
  ...
  janePlays <-
    Lotto.play
      (fst alicePlays)
      seal
      (snd alicePlays ^. Cooked.outputValueL)
      (Lib.hashSecret "jane" (Just salt))
      jane
      (Lib.ada 10)
  void $ Lotto.resolve secret janePlays seal


playAfter :: Cooked.MonadModalBlockChain m =>
--  Lotto.Setup ->
  -- | Salt
  BuiltinByteString ->
  -- | Secret
...
playAfter salt secret seal last w = do
    walletPlays <-
      Lotto.play
      (fst last)
      seal
      (snd last ^. Cooked.outputValueL)
      (Lib.hashSecret "jane" (Just salt)) -- fix later not relevant right now
      w
      (Lib.ada 10)
    return walletPlays

{-
guesses :: [BuiltinByteString]
guesses = ["alice", "bob", "carol", "chuck", "dan", "eve", "frank", "grace", "heidi", "ivan", "jane"]

randomGuess :: (MonadRandom m) => m BuiltinByteString
randomGuess = do
...
multiplePlays :: Cooked.MonadModalBlockChain m =>
--  Lotto.Setup ->
  -- | Salt
  BuiltinByteString ->
  -- | Secret
  ...
  [Cooked.Wallet]  ->
  m (LedgerV2.TxOutRef, LedgerV2.TxOut)
multiplePlays salt secret seal last [] = return last
multiplePlays salt secret seal last (w:ws) = do
                                                  res <- playAfter salt secret seal last w
                                                  multiplePlays salt secret seal res ws

-- | A trace where a list of wallets play
manyWalletPlay ::
  Cooked.MonadModalBlockChain m =>
  Lotto.Setup ->
  -- | Salt
